...
my $SCORING    = 2;
my $TEAM_STATS = 1;
my $ALLDATA = $TEAM_STATS + $SCORING + $BOXSCORE;
...
# Team stats callbacks
sub parse_json_teamStats_first_downs($$);
sub parse_json_teamStats_fumbles($$);

my %BOXSCORE_CALLBACKS = (
  "kicking"      => \&parse_json_boxscore_kicking,
  "kick_returns" => \&parse_json_boxscore_kick_returns,
  "passing"      => \&parse_json_boxscore_passing,
  "punt_returns" => \&parse_json_boxscore_punt_returns,
  "punting"      => \&parse_json_boxscore_punting,
  "rushing"      => \&parse_json_boxscore_rushing,
);

my %TEAM_STATS_CALLBACKS = (
  "1st Downs" => \&parse_json_teamStats_first_downs,
  "Fumbles: Number-Lost" => \&parse_json_teamStats_fumbles,
  "Interception Returns: Number-Yards" => \&parse_json_teamStats_interceptions,
  "Penalties: Number-Yards" => \&parse_json_teamStats_penalties,
);
...
# ScorePlays: FGM + TDs
# Need to get:
# - FGA          Box ~> kicking ~> FG-FGA
my $FGA = "FGA";
# - FGM          Box ~> kicking ~> FG-FGA
my $FGM = "FGM";
# - KORetYards   Box ~> kick_returns ~> NO
my $KORetYards = "KORetYards";
# - PassPlays    Box ~> passing ~> CP-ATT-INT
my $PassPlays = "PassPlays";
# - PassYards    Box ~> passing ~> YDS
my $PassYards = "PassYards";
# - PuntRetYards Box ~> punt_returns ~> YDS
my $PuntRetYards = "PuntRetYards";
# - Punts        Box ~> punting ~> NO
my $Punts = "Punts";
# - RushPlays    Box ~> rushing ~> ATT
my $RushPlays = "RushPlays";
# - RushYards    Box ~> rushing ~> YDS
my $RushYards = "RushYards";
# - TDs          Scoring ~> quarter ~> summary ~> scoreType==TD
my $TDs = "TDs";
# - NumPoints    Scoring ~> last visiting/home score
my $NumPoints = "NumPoints";
# - Fumbles      TeamStats ~> Fumbles: Number-Lost
my $Fumbles = "Fumbles";
# - IntRetYards  TeamStats ~> Interception Returns: Number-Yards
my $IntRetYards = "IntRetYards";
# - Num1stDowns  TeamStats ~> 1st Downs
my $Num1stDowns = "Num1stDowns";
# - NumIntercept Box ~> passing ~> CP-ATT-INT
my $NumIntercept = "NumIntercept";
# - NumPenalties TeamStats ~> Penalties: Number-Yards
my $NumPenalties = "NumPenalties";
my @ALLSTATS = ($FGA, $FGM, $KORetYards, $PassPlays, $PassYards,
    $PuntRetYards, $Punts, $RushPlays, $RushYards, $TDs, $NumPoints,
    $Fumbles, $IntRetYards, $Num1stDowns, $NumIntercept, $NumPenalties);
my %gamedata;
foreach my $f (@ARGV) {
  my $ftype = 0;
  my $gid = undef;
  ...
  if ($f =~ /_teamStats.json$/) {
    $ftype = $TEAM_STATS;
  } elsif ($f =~ /_scoringSummary.json$/) {
    $ftype = $SCORING;
  } elsif ($f =~ /_boxscore.json$/) {
    $ftype = $BOXSCORE;
  } else {
#    warn "Unknown filetype: $f\n";
    next;
  }
...
sub load_json($$$) {
  ...
  close $fh;
  if (!defined($raw_json) or !length($raw_json)) {
    warn "Invalid/empty JSON in $filename";
    return;
  }
  my $json_data = decode_json($raw_json);
  if (!defined($json_data)) {
    warn "Invalid JSON in $filename";
    return;
  }
  if ($filetype == $BOXSCORE) {
    process_json_boxscore($gid, $json_data);
  } elsif ($filetype == $SCORING) {
    process_json_scoringSummary($gid, $json_data);
  } elsif ($filetype == $TEAM_STATS) {
    process_json_teamStats($gid, $json_data);
  } else {
    warn "Invalid filetype $filetype";
    return;
  }
}
...
# - FGA          Box ~> kicking ~> FG-FGA
# - FGM          Box ~> kicking ~> FG-FGA
# - KORetYards   Box ~> kick_returns ~> YDS
# - NumIntercept Box ~> passing ~> CP-ATT-INT
# - PassPlays    Box ~> passing ~> CP-ATT-INT
# - PassYards    Box ~> passing ~> YDS
# - PuntRetYards Box ~> punt_returns ~> YDS
