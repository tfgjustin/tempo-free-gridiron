...
def is_valid_row(row):
  if 'LineType' not in row:
    return False
  return row['LineType'] == 'Spread' or row['LineType'] == 'Total'
...
def calculate_winprob(line_value, pred_value, pred_dev):
  ...
  if pred_dev is None or pred_dev == '_':
    return 0
  # In this case the predicted value is how much they should win (or lose) by,
  # and the line is the adjustment. So if the edge is positive then they should
  # win the bet. Here we reduce the edge by 7.5, meaning there has to be a
  # minimum of a TD padding to even make a bet.
  edge = (float(pred_value) + float(line_value)) - 7.5
  edge = max(1e-5, edge)
...
def calculate_kelly(row_key, odds_row, system_row):
  ...
  if odds_row['LinePrice'] == '_' or odds_row['LinePrice'] == '0':
    return None
  pred_value = None
  if 'Spread' in system_row:
    pred_value = system_row['Spread']
  if not pred_value:
    return None
  win_prob = calculate_winprob(odds_row.get('LineValue', None),
                               pred_value, system_row.get('Dev', None))
  kelly_val = betlib.kellyCriterion(win_prob, line=int(odds_row['LinePrice']))
  if kelly_val < 0:
    return 0
  return kelly_val
