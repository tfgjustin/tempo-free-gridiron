...
__author__ = 'justin@tfgridiron.com (Justin Moore)'
...
def PrintBlogPage(page):
  print '%s,%s,%s' % (page['title'].replace(',', ' '), page['url'], page['id'])

def PrintBlogPages(http, pages, token):
  req = pages.list(blogId=TFG_BLOG_ID, pageToken=token,
...
#    return
#  PrintBlogPages(http, pages, resp['nextPageToken'])
  return
...
def main(argv):
  ...
  labels = []
  try:
    PrintBlogPages(http, pages, None)
